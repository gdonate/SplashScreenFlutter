...
class AnimacionParticulas extends StatefulWidget{
  ...
  const AnimacionParticulas(this.numberOfParticles, {Key? key}) : super(key: key);
...
class _AnimacionParticulasState extends State<AnimacionParticulas>
  ...
  _simulateParticles(){
    for (var particle in particles){
      particle.checkIfParticlesNeedsToBeRestarted();
    }
  }
...
class ParticleModel {
  ...
  restart(){
    ...
    duration = 3.seconds + Random().nextInt(3000).milliseconds;
  ...
  checkIfParticlesNeedsToBeRestarted(){
    if (progress() == 1.0){
      restart();
    }
  }
